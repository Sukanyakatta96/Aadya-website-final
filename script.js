...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const paths = {
    east: '/brochures/east-facing.pdf',
    west: '/brochures/west-facing.pdf'
  };
  ...
  if (!agree.checked) {
    ...
    try {
      ...
      a.href = url;
      a.download = type === 'east' ? 'east-facing.pdf' : 'west-facing.pdf';
...
function showGreeting() {
    ...
    addMessage(`${greeting} I'm here to help you find your dream villa. Let's begin your journey to a dream home.`);
...
function showMainMenu() {
    ...
    const menuHtml = `
        <div class="quick-btns">
            <button class="quick-btn" onclick="showProjects()">🏠 Explore Villas</button>
...
function showProjects() {
    ...
    const projectsHtml = `
        <div class="project-card">
            <h4>🌅 East Facing Villas</h4>
            <div class="project-actions">
                <button class="action-btn" onclick="downloadBrochure('east')">📄 Download Brochure</button>
                <button class="action-btn" onclick="shareProject('east', 'email')">📧 Email</button>
                <button class="action-btn" onclick="shareProject('east', 'whatsapp')">💬 WhatsApp</button>
            </div>
        </div>
        <div class="project-card">
            <h4>🌇 West Facing Villas</h4>
            <div class="project-actions">
                <button class="action-btn" onclick="downloadBrochure('west')">📄 Download Brochure</button>
                <button class="action-btn" onclick="shareProject('west', 'email')">📧 Email</button>
                <button class="action-btn" onclick="shareProject('west', 'whatsapp')">💬 WhatsApp</button>
            </div>
        </div>
        <button class="back-btn" onclick="showMainMenu()">← Back to Menu</button>
    `;
    addMessage("Explore our exclusive villa collection:", true);
    addMessage(projectsHtml, true, true);
}
...
function downloadBrochure(type) {
    const villaType = type === 'east' ? 'East Facing' : 'West Facing';
    const pdfPath = type === 'east' ? 'assets/east-villa.pdf' : 'assets/west-villa.pdf';
    addMessage(`📄 Downloading ${villaType} Villa brochure...`, 'bot');
    try {
        // Create hidden link to force download
        const link = document.createElement('a');
        link.href = pdfPath;
        link.download = pdfPath.split('/').pop(); // e.g. east-villa.pdf
        link.style.display = "none";
        document.body.appendChild(link);
        link.click(); // triggers download
        document.body.removeChild(link);
        // Continue bot workflow without redirect
        setTimeout(() => {
            addMessage(`✅ ${villaType} Villa brochure downloaded successfully!`, 'bot');
            addMessage("Would you like to schedule a site visit or need more information?", 'bot');
            const followUpHtml = `
                <div class="quick-btns">
                    <button class="quick-btn" onclick="scheduleVisit('${type}')">📅 Schedule Visit</button>
        ...
        addMessage("⚠️ Failed to download brochure. Please try again later.", "bot");
    }
}

function shareProject(type, method) {
    const villaType = type === 'east' ? 'East Facing' : 'West Facing';
    const message = `Check out this amazing ${villaType} Villa project by Aadya Real Estate!`;
    const url = window.location.href;
    if (method === 'email') {
        const subject = encodeURIComponent(`${villaType} Villa - Aadya Real Estate`);
        const body = encodeURIComponent(`${message}\n\nProject Details:\n- RERA Approved\n- Premium Location\n- Modern Amenities\n- Easy Finance Options\n\nVisit: ${url}`);
        window.open(`mailto:?subject=${subject}&body=${body}`);
        addMessage(`📧 Email sharing opened for ${villaType} Villa`);
    } else if (method === 'whatsapp') {
        const text = encodeURIComponent(`${message}\n\nProject Highlights:\n✅ RERA Approved\n🏡 Premium Amenities\n📍 Prime Location\n💰 Easy Finance\n\n${url}`);
        window.open(`https://wa.me/?text=${text}`, '_blank');
        addMessage(`💬 WhatsApp sharing opened for ${villaType} Villa`);
    }
}

function scheduleVisit(type) {
    const villaType = type === 'east' ? 'East Facing' : 'West Facing';
    addMessage(`📅 Great choice! I'll help you schedule a visit to our ${villaType} Villa project.`);
    const scheduleHtml = `
        <div class="contact-form">
            ...
            <div class="quick-btns">
                <button class="quick-btn" onclick="confirmSchedule('${type}', 'today')">📅 Today</button>
                <button class="quick-btn" onclick="confirmSchedule('${type}', 'tomorrow')">📅 Tomorrow</button>
                <button class="quick-btn" onclick="confirmSchedule('${type}', 'weekend')">📅 This Weekend</button>
            </div>
        </div>
    `;
...
function confirmSchedule(type, timeframe) {
    const villaType = type === 'east' ? 'East Facing' : 'West Facing';
    let timeText = '';
    switch(timeframe) {
    ...
    addMessage(`✅ Perfect! Your site visit for ${villaType} Villa is scheduled for ${timeText}.`);
